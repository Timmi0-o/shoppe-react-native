...
export const useOrder = () => {
	...
	const { data: allOrders }: SWRResponse<OrderData> = useSWR(
		() => (user ? { url: `${BACK_PORT}order/${user?._id}` } : null),
		fetcher
	)
	...
	const { data: oneOrder }: SWRResponse<OrderProductData> = useSWR(
		() =>
			user ? { url: `${BACK_PORT}order/${user?._id}/${orderNumber}` } : null,
		fetcher
	)
	...
	const handleNewOrder = async (order: AddNewOrderData) => {
		try {
			const response = await axios.patch(`${BACK_PORT}order/add`, {
				...order,
			})
			if (response.data) {
				await axios.patch(`${BACK_PORT}basket/clear/${order.user}`)
